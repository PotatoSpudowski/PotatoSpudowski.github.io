    ...
    <main className="main blog-main">
      ...
      <div className="blog-body">
        ...
        <section className="blog-section">
          ...
          <p className="blog-p">the direction method stopped at 30 percent refusals on gemma 4 E4B. the same method reached 0 percent on Qwen3.6-35B. the difference is not better alignment. the difference is architecture. gemma 4 is fault-tolerant. we read the E4B configuration to find the cause. the configuration shows three architectural properties that dilute every weight edit.</p>
          ...
          <p className="blog-p">attention applies <code>q_norm</code> and <code>k_norm</code> after the projections. these are per-head RMSNorms at head dimension 256. the value path uses <code>v_norm</code> without a learned scale. a magnitude change in the K and V weights is normalized away before attention runs. only a direction change in K and V survives.</p>
          <p className="blog-p">this geometry has three consequences. first: LoRA adapters and inference-time steering hooks produce zero behavioral change. the norms absorb the perturbation. direct weight editing is the only option. second: weak edits fail. the edit must rotate weight rows past orthogonal, so the model anti-expresses the refusal direction. third: editing K and V is pointless. the direction change cannot survive the per-head norms either.</p>

          <p className="blog-p"><strong>property 2: per-layer embeddings.</strong> the configuration field <code>hidden_size_per_layer_input</code> is 256. the model has a second embedding table, <code>embed_tokens_per_layer</code>, with shape (262144, 8960). that is 35 layers times 256 dimensions. each decoder layer receives its own 256-dimensional embedding vector. the vector passes through <code>per_layer_input_gate</code> (256 by 1536), a gating product with the residual stream, and <code>per_layer_projection</code> (1536 by 256). the result is added to the residual stream after the normalization layers. this channel bypasses every matrix that standard abliteration edits.</p>
